...
3. Sections: **prebuilt only** (Header, Hero, About, Skills, Projects/Portfolio, CTA, Experience, Education, Contact, Footer) — no custom section types; each type at most once; reorder + hide allowed. Hero, About, Skills, Projects, and CTA offer **6 layout variants**; Header is fixed (logo + section nav); Footer is fixed (centered copyright). Header & Footer are pinned.
